...
from utils import get_body_text, dump_json, load_json
...
def get_matches_in_bodies(paths: list[str], min_match_size=100, sequence_matcher_autojunk: bool = True) -> list[tuple[str, list]]:
    completed_doc_pairs = set()
    matches_with_paths_d = {}

    for path1 in tqdm(paths):
        body1 = get_body_text(path1)
        if not body1:
            continue
...
def preprocess_doc_bodies(
    ...
    Args:
        doc_ref (DataFrame): _description_
    ...
    for path in doc_ref[doc_ref['embed_index'] >= 0]['path']:
